...
module PlacesHelper
  ...
  #
  # @param pla [Place]
  # @param hyperlink: [Boolean, Symbol] If true (Def), Hyperlink to Place is included IF Place is readable for the user. If :Prefecture or :all, hyperlink to Prefecture is also included if permitted.
  ...
  def show_pref_place_country(pla, hyperlink: true, prefer_shorter: false, lang_fallback_option: :either, article_to_head: true, **kwd)
    ...
    pref_title =
      if !pref
        "UNDEFINED"  # should never happen in practice...
      elsif [:Prefecture, :all].include?(hyperlink) && can?(:read, pref)
        link_to (ar[1].blank? ? "NO-TRANSLATION" : ar[1]), pref, data: {turbo: false}
      else
        h(ar[1])
      end
    ...
    sprintf('%s %s(%s)', pref_title, pla_title, h(ar[2])).html_safe
  end
end
