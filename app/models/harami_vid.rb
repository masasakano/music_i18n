  ...
  private    ################### including Callbacks
    ...
    def _determine_musics_from_csv(hsrow, org_line, arts, art_tit, iline: nil)
      ...
      when 1
        ...
        music = muss2.first
        s_link = ActionController::Base.helpers.link_to(mu_tit.inspect,
                                                        Rails.application.routes.url_helpers.polymorphic_path(music, locale: I18n.locale),
                                                        title: "pID=#{music.id}")
      ...
      if record.note.blank?
        ...
        tgtpath = 
          if record.respond_to?(:music)  # HaramiVidMusicAssoc (possibly also ArtistMusicPlay in the future?)
            ...
            Rails.application.routes.url_helpers.polymorphic_path(self, locale: I18n.locale) + obj_dom_id  # HaramiVid-show (NOTE: the current CREATE path differs!)
          else
            Rails.application.routes.url_helpers.polymorphic_path(record, locale: I18n.locale)
          end
