...
#           print "DEBUG:abi03: ";p(i.user.an_admin? && (!uhrc || uhrc && i.user.highest_role_in(hrc) < uhrc))
      ...
      can :read, [Instrument, ChannelType, SiteCategory, DomainTitle]
    end
    ...
    if user.qualified_as?(:moderator, rc_general_ja)
      ...
      can([:crud], SiteCategory){|mdl| (!mdl.unknown? && "main" != mdl.mname) }  # can create/update/destroy except for "unknown" (though cannot destroy if there's a dependent DomainTitle).  At the time of writing, SiteCategory.default(:HaramiVid) returns SiteCategory.unknown.
      can([:crud], DomainTitle){|mdl| !mdl.unknown? }  # can create/update/destroy except for "unknown" (though cannot destroy if there's a dependent Uri or Domain).
    end
    ...
    cannot(:destroy, [SiteCategory, DomainTitle]){                  |mdl| mdl.unknown?}  # can be managed by only sysadmin
  end
end
