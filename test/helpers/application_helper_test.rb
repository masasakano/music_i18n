...
class ApplicationHelperTest < ActionView::TestCase
  ...
  test "AH.sorted_title_ids" do
    org_locale = I18n.locale
    begin
      I18n.locale = "en"
      ary = AH.sorted_title_ids(ChannelOwner.all, method: :title_or_alt_for_selection_optimum).map(&:first)
      ...
      ary = AH.sorted_title_ids(rela_ctype, method: :title_or_alt_for_selection_optimum).map(&:first)
      assert_equal "Side channel", ary[0]
      ary = AH.sorted_title_ids(rela_ctype, method: :title_or_alt, langcode: I18n.locale).map(&:first)
      assert_equal "Side channel", ary[0]
      ary = AH.sorted_title_ids(rela_ctype, method: :title_or_alt, langcode: I18n.locale, prefer_shorter: true).map(&:first)
