...
class ArtistsTest < ApplicationSystemTestCase
  ...
  test "visiting Artist#index" do
    ...
    assert_selector "h1", text: @h1_title
    assert_selector "table", text: "ハラミちゃん"
    stats3 = _get_stats
    ...
    assert_operator stats1[:n_cur], :>, stats3[:n_cur], page.find('p.pagenation_stats')['outerHTML']+"stats1(original)=#{stats1[:n_cur].inspect}\nstats3(filteri)=#{stats3[:n_cur].inspect}"  # Number decreased after filtering. ## NOTE: If this fails, the server's response is too slow and may be still processing the submission; to avoid it, you would need assert_selector to match the NOT-disabled <input type="submit" name="commit" value="Apply" class="datagrid-submit" data-disable-with="Apply"> (or "適用"). However, I do not know the exact CSS/XPath for assert_selector to match, yet. The error message here should print out?  Maybe:  assert_selector('input[type="submit"]:not([disabled])')  or "visible: true" (??)
  ...
  test "should create/update artist" do
    ...
    user = @moderator_all  # users(:user_syshelper)
    login_from_somewhere(user.email, from: :signin)  # defined in test_system_helper.rb
    assert_equal(user.display_name, current_user_display_name(is_system_test: true))  # defined in test_helper.rb
    ...
    page.using_wait_time(0){
      ...
      assert_empty find_all(:xpath, xpath_fr_row)
    ...
    assert_equal user, art_trans1.create_user
    assert_equal user, art_trans1.update_user

    # Artificially creates another Translation (in fr) for the created Artist and revisits the page
    title_fr = "Dummy2"
    art_trans2 = Translation.create!(title: title_fr, langcode: "fr", is_orig: false, weight: 1000, translatable: new_artist)
    assert_equal 2, new_artist.translations.reset.count
    assert_equal user, art_trans2.create_user
    assert_equal user, art_trans2.update_user
    ...
    refute_equal user, art_trans1.create_user
    refute_equal user, art_trans1.update_user
...
#take_screenshot
  ...
  ## NOTE: See musics_test.rb for test-"Music#destroyable?" for slightly better tests
  test "destroying Artist in Show" do
    art_title = "some artist of #{__method__}"
    new_artist = Artist.create_basic!(title: art_title, langcode: :en, is_orig: true, birth_year: 1950, note: 'temporary new_artist')

    css_destroy_button_in_show_min = sprintf(".link-edit-destroy .destroy_link")
    css_destroy_button_in_show_strict = sprintf(".%s #%s .destroy_link", get_only_css_class, Consts::HtmlIds::MAIN_LINK_EDIT_MERGE_DESTROY) # defined in application_helper.rb

    ## Visits the Artist page by Public
    visit artist_path(new_artist)
    assert_selector "h1", text: "Artist: "+art_title

    refute_selector css_destroy_button_in_show_min  # destroy-button should not be visible, but...
    refute_selector css_destroy_button_in_show_min, text: "Destroy"
    refute_selector css_destroy_button_in_show_strict, text: "Destroy"
    refute_text "Destroy"

    login_from_somewhere(@editor_ja, from: :visit)
    visit artist_path(new_artist)
      # authenticated and authorized

    assert_selector "h1", text: "Artist: "+art_title
    assert_selector    "dt", text: "Channel Owner"
    assert_selector css_destroy_button_in_show_min  # destroy-button should be visible, but...
    assert_selector css_destroy_button_in_show_min, text: "Destroy"
    assert_selector css_destroy_button_in_show_strict, text: "Destroy"
    ...
    new_music.reload 
    new_artist.reload
    assert new_artist.musics.exists?, "sanity check"
    ...
    new_hvid.musics << new_music  # creating join-record HaramiVidMusicAssoc
    ...
    # Destroys "Artist<=>EventItem/Music" association, thus HaramiVid association.
    ...
    assert_difference("Artist.count", -1){
      assert_destroy_with_text(find(css_destroy_button_in_show_strict))  # defined in test_system_helper.rb
      assert_selector "h1", text: "Artists"
    }
  end
