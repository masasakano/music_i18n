...
class DeviseIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "get create user registration" do
    assert_equal '/en/users/sign_up', new_user_registration_path.sub(/\?.+/, '')
    assert_equal '/ja/users/sign_up', new_user_registration_path(locale: "ja")
    ...
    get new_user_registration_path(locale: "ja")
    ...
    # assert_includes css.text, I18n.t('read_agree_terms_html', locale: :en).sub(/\s+%\{[^\}]+\}.+/, "")  # The original English term is like "...and agreed with the %{link2term}."
    ...
    assert_difference('User.count', 1){
      ...
      assert_response :success  # meaning failure - has to retry
      ### For some reason, the following is in English in Rails-8.1.....
