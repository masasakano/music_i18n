...
if false    
    ...
    _assert_authorized_get_set(paths[:new], Anchoring, fail_users: [], success_users: [@editor_ja, @editor_harami], h1_title_regex: nil, bind_offset: def_bind_offset+4)

    _assert_authorized_get_set(paths[:edit],Anchoring, fail_users: [], success_users: [@editor_ja, @editor_harami], h1_title_regex: nil, bind_offset: def_bind_offset+4)

    # assert_equal :create, assert_unauthorized_post(Anchoring, user: nil, params: hs2pass5, path_or_action: paths[:create], bind_offset: def_bind_offset) # defined in /test/helpers/controller_helper.rb
    # end

    sign_in @moderator_ja

    ## Successful creation of Anchoring and Url with existing Domain and DomainTitle
    action, new_mdl2 = assert_authorized_post(Anchoring, user: @moderator_ja, path_or_action: paths[:create], redirected_to: proc_art0_path, params: hsprms, method: :post, diff_count_command: calc_count_exp, diff_num: 11001, bind_offset: def_bind_offset){ |_, _| # defined in /test/helpers/controller_helper.rb
    ...
    action, new_mdl3 = assert_authorized_post(Anchoring, user: @moderator_ja, path_or_action: paths[:create], redirected_to: proc_art0_path, params: hsprms3, method: :post, diff_count_command: calc_count_exp, diff_num: 12111, updated_attrs: %i(note), bind_offset: def_bind_offset){ |_, record| # defined in /test/helpers/controller_helper.rb
    ...
    action, new_mdl4 = assert_authorized_post(Anchoring, user: @moderator_ja, path_or_action: paths[:create], redirected_to: proc_art0_path, params: hswiki, method: :post, diff_count_command: calc_count_exp, diff_num: 11001, bind_offset: def_bind_offset){ |user, record| # defined in /test/helpers/controller_helper.rb
...
#
#    assert_equal :create, assert_authorized_post(Url, params: hs2pass.merge({title: ""}), diff_num: 0, err_msg: "null title should not be allowed, but Response is...", bind_offset: def_bind_offset).first # defined in /test/helpers/controller_helper.rb
#
#    assert_equal :create, assert_authorized_post(Url, params: hs2pass2.merge({url: "abc.x/invalid" }), diff_num: 0, err_msg: "should have failed due to the invalid URI, but Response is...", bind_offset: def_bind_offset).first

    ### update/patch

#    [nil, @translator].each do |ea_user|  # @trans_moderator may be pviviledged?
#      assert_unauthorized_post(anchoring, user: ea_user, path_or_action: paths[:show], params: hsprms, method: :patch, diff_count_command: calc_count_exp, bind_offset: def_bind_offset){ |user, record|
